...
dataset_location = 'data/user_tracks_completed_subset_100k.tsv'
random.seed(42)
...
def split(test_size):
    ...
    for line in tqdm.tqdm(open(dataset_location)):
        ...
        curr_time = datetime.strptime(hists[4], '%Y-%m-%d').timestamp()
    ...
    print ('TOTAL ARTISTS:', len([a for a in artists_users.keys() if len(artists_users[a]) > numb_user_p_artist]))
    ...
    for line in tqdm.tqdm(open(dataset_location)):
        ...
        if user_pos[last_user] == count:
            ...
            for item, engagement, awearnes, play, different_songs, days in artist_fan:
                if play > 0:# or (random.uniform(0, 1) > 0.5): # Ignore when the user listened less than X times
            ...
            """
            if last_user in user_test_split:
                for item, engagement, awearnes, play in test_artist_fan:
                    if item not in fan_items_dict:
                        fan_items_dict[item] = len(fan_item_ids)
                        fan_item_ids.append(item)
                    fan_col_train.append(fan_items_dict[item])
...
if __name__== "__main__":
    ...
    sparse.save_npz(os.path.join('data', 'pandora', 'fan_train_data_awearnes_features_fidelity_05.npz'), fan_train_awe)
    sparse.save_npz(os.path.join('data', 'pandora', 'fan_train_data_playcount_features_fidelity_05.npz'), fan_train_play)
    sparse.save_npz(os.path.join('data', 'pandora', 'fan_train_data_engagement_features_fidelity_05.npz'), fan_train_eng)
    sparse.save_npz(os.path.join('data', 'pandora', 'fan_train_data_days_features_fidelity_05.npz'), fan_train_days)
    pickle.dump(fan_test_data, open(os.path.join('data', 'pandora','fan_test_data_features_fidelity_05.pkl'), 'wb'))
    pickle.dump(fan_items_dict, open(os.path.join('data','pandora', 'fan_items_dict_features_fidelity_05.pkl'), 'wb'))
    pickle.dump(fan_users_dict, open(os.path.join('data','pandora', 'fan_users_dict_features_fidelity_06.pkl'), 'wb'))
