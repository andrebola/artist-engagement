...
dataset_location = '../noemi-session-rec/data/nowplaying/raw/nowplaying.csv'
random.seed(42)
...
def split(test_size):
    ...
    for line in tqdm.tqdm(open(dataset_location)):
        ...
        if hists[4] not in artists_catalog:
            artists_catalog[hists[4]] = set()
        artists_catalog[hists[4]].add(hists[2])
        if hists[4] not in artists_users:
            artists_users[hists[4]] = set()
        artists_users[hists[4]].add(hists[0])
        curr_time = int(hists[3])
    ...
    print ("artists: ", len(artists_catalog))
    ...
    for line in tqdm.tqdm(open(dataset_location)):
        ...
        if curr_time < split_time:
            if hists[4] not in counts_dict[hists[0]]:
                counts_dict[hists[0]][hists[4]] = {'s':set(), 't': 0, 'times':[]}
            counts_dict[hists[0]][hists[4]]['s'].add(hists[2])
            counts_dict[hists[0]][hists[4]]['t'] += 1
            counts_dict[hists[0]][hists[4]]['times'].append(curr_time)
        else:
            if hists[0] not in user_test_split:
                user_test_split[hists[0]] = {}
            if hists[4] not in user_test_split[hists[0]]:
                user_test_split[hists[0]][hists[4]] = {'s':set(), 't': 0, 'times':[]}
            user_test_split[hists[0]][hists[4]]['s'].add(hists[2])
            user_test_split[hists[0]][hists[4]]['t'] += 1
            user_test_split[hists[0]][hists[4]]['times'].append(curr_time)
            #if hists[0] not in user_test_split:
            #    user_test_split[hists[0]] = []
...
if __name__== "__main__":
    ...
    sparse.save_npz(os.path.join('data', 'nowplaying_time', 'fan_train_data_awearnes_features_fidelity_05.npz'), fan_train_awe)
    sparse.save_npz(os.path.join('data', 'nowplaying_time', 'fan_train_data_playcount_features_fidelity_05.npz'), fan_train_play)
    sparse.save_npz(os.path.join('data', 'nowplaying_time', 'fan_train_data_engagement_features_fidelity_05.npz'), fan_train_eng)
    sparse.save_npz(os.path.join('data', 'nowplaying_time', 'fan_train_data_days_features_fidelity_05.npz'), fan_train_days)
    pickle.dump(fan_test_data, open(os.path.join('data', 'nowplaying_time','fan_test_data_features_fidelity_05.pkl'), 'wb'))
    pickle.dump(fan_items_dict, open(os.path.join('data','nowplaying_time', 'fan_items_dict_features_fidelity_05.pkl'), 'wb'))
    pickle.dump(fan_users_dict, open(os.path.join('data','nowplaying_time', 'fan_users_dict_features_fidelity_05.pkl'), 'wb'))
